...
public class MKIPathPlanner {
     ...
     */
    public static MKIPathPlannerTrajectory loadPath(String name, double maxVel, double maxAccel, boolean reversed) {
        try(BufferedReader br = new BufferedReader(new FileReader(new File(Filesystem.getDeployDirectory(), "pathplanner/" + name + ".path")))){
            ...
            ArrayList<MKIPathPlannerTrajectory.Waypoint> waypoints = new ArrayList<>();
            ...
            for (Object waypoint : jsonWaypoints) {
                ...
                Translation2d anchorPoint = new Translation2d((double) jsonAnchor.get("x"), -1*(double) jsonAnchor.get("y"));

                JSONObject jsonPrevControl = (JSONObject) jsonWaypoint.get("prevControl");
                Translation2d prevControl = null;
                if (jsonPrevControl != null) {
                    prevControl = new Translation2d((double) jsonPrevControl.get("x"), -1*(double) jsonPrevControl.get("y"));
                }

                JSONObject jsonNextControl = (JSONObject) jsonWaypoint.get("nextControl");
                Translation2d nextControl = null;
                if (jsonNextControl != null) {
                    nextControl = new Translation2d((double) jsonNextControl.get("x"), -1*(double) jsonNextControl.get("y"));
                }
                ...
                waypoints.add(new MKIPathPlannerTrajectory.Waypoint(anchorPoint, prevControl, nextControl, velOverride, holonomicAngle, isReversal));
            }

            ArrayList<ArrayList<MKIPathPlannerTrajectory.Waypoint>> splitPaths = new ArrayList<>();
            ArrayList<MKIPathPlannerTrajectory.Waypoint> currentPath = new ArrayList<>();

            for(int i = 0; i < waypoints.size(); i++){
                MKIPathPlannerTrajectory.Waypoint w = waypoints.get(i);
            ...
            ArrayList<MKIPathPlannerTrajectory> paths = new ArrayList<>();
            boolean shouldReverse = reversed;
            for(int i = 0; i < splitPaths.size(); i++){
                paths.add(new MKIPathPlannerTrajectory(splitPaths.get(i), maxVel, maxAccel, shouldReverse));
     ...
     * @return The generated path
     */
    public static MKIPathPlannerTrajectory loadPath(String name, double maxVel, double maxAccel){
        return loadPath(name, maxVel, maxAccel, false);
    }

    private static MKIPathPlannerTrajectory joinPaths(ArrayList<MKIPathPlannerTrajectory> paths){
        ...
        return new MKIPathPlannerTrajectory(joinedStates);
    }
}