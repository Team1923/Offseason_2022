...
public class MKIPathPlannerTrajectory extends Trajectory {
    protected MKIPathPlannerTrajectory(ArrayList<Waypoint> pathPoints, double maxVel, double maxAccel, boolean reversed){
        super(generatePath(pathPoints, maxVel, maxAccel, reversed));
    }

    protected MKIPathPlannerTrajectory(List<State> states){
        super(states);
    }
    ...
    private static List<State> generatePath(ArrayList<Waypoint> pathPoints, double maxVel, double maxAccel, boolean reversed){
        List<PathPlannerState> joined = joinSplines(pathPoints, maxVel, MKIPathPlanner.resolution);
    ...
    private static ArrayList<PathPlannerState> joinSplines(ArrayList<Waypoint> pathPoints, double maxVel, double step){
        ...
        for(int i = 0; i < numSplines; i++){
            ...
            for(double t = 0; t <= endStep; t += step){
                Translation2d p = MKIGeometryUtil.cubicLerp(startPoint.anchorPoint, startPoint.nextControl, endPoint.prevControl, endPoint.anchorPoint, t);
    ...
    public static class PathPlannerState extends State{
        ...
        private PathPlannerState interpolate(PathPlannerState endVal, double t){
            ...
            lerpedState.timeSeconds = MKIGeometryUtil.doubleLerp(timeSeconds, endVal.timeSeconds, t);
            ...
            lerpedState.velocityMetersPerSecond = MKIGeometryUtil.doubleLerp(velocityMetersPerSecond, endVal.velocityMetersPerSecond, t);
            lerpedState.positionMeters = (velocityMetersPerSecond * deltaT) + (0.5 * accelerationMetersPerSecondSq * Math.pow(deltaT, 2));
            lerpedState.accelerationMetersPerSecondSq = MKIGeometryUtil.doubleLerp(accelerationMetersPerSecondSq, endVal.accelerationMetersPerSecondSq, t);
            Translation2d newTrans = MKIGeometryUtil.translationLerp(poseMeters.getTranslation(), endVal.poseMeters.getTranslation(), t);
            Rotation2d newHeading = MKIGeometryUtil.rotationLerp(poseMeters.getRotation(), endVal.poseMeters.getRotation(), t);
            lerpedState.poseMeters = new Pose2d(newTrans, newHeading);
            lerpedState.angularVelocity = MKIGeometryUtil.rotationLerp(angularVelocity, endVal.angularVelocity, t);
            lerpedState.angularAcceleration = MKIGeometryUtil.rotationLerp(angularAcceleration, endVal.angularAcceleration, t);
            lerpedState.holonomicRotation = MKIGeometryUtil.rotationLerp(holonomicRotation, endVal.holonomicRotation, t);
            lerpedState.curveRadius = MKIGeometryUtil.doubleLerp(curveRadius, endVal.curveRadius, t);
            lerpedState.curvatureRadPerMeter = MKIGeometryUtil.doubleLerp(curvatureRadPerMeter, endVal.curvatureRadPerMeter, t);
