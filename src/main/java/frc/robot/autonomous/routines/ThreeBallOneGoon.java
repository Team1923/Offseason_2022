...
public class ThreeBallOneGoon extends SequentialCommandGroup {
  /** Creates a new DeuxBall. */
  public ThreeBallOneGoon(SwerveSubsystem swerve, ShooterSubsystem shooter, ConveyorSubsystem conveyor, IntakeSubsystem intake, HoodSubsystem hood, LimelightSubsystem limelight) {
    ...
    addCommands(
      ...
      new VisionTrack(swerve, () -> fake(), ()-> fake(), limelight).withTimeout(0.5),
      new AutoShoot(shooter, conveyor, hood, UnitConversion.angleToTicks(24.5), 3200).withTimeout(1.5),
      new ParallelCommandGroup(
        new RunIntakeCommand(intake, false),
        new RunTrajectory(swerve, "threeBallGoon", false)
      ).withTimeout(2.5),
      new PIDRotate(swerve, -30).withTimeout(1),
      new RunIntakeCommand(intake, true).withTimeout(1.5)
    );
  }
